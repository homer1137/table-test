...
export const Table = (props: Props) => {
  ...
  const LinesChild = ({ line }: { line: ILine }) => {
    return (
      <>
        ...
        {line.child &&
          line.child.map((lineChild: ILine, ) => (
            <div key={lineChild.id} className={"table__child"}>
              <TableItem line={lineChild} key={lineChild.id} />
              {lineChild.child &&
                lineChild.child.map((lineChild2: ILine) => (
                  <LinesChild line={lineChild2} key={lineChild2.id} />
                ))}
            </div>
          ))}
      </>
  ...
  return (
    <div className="table">
      ...
      <div className="table__tableHeader">Строительно-монтажные работы</div>
      <div className="table__tableHeader2"></div>
